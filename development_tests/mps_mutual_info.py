...
def mutual_information(data, idx, num_terms=5, weights=None):
    ...
    initial_model = Unbounded_Metalog_Model(init_a=torch.ones([channels, 7])).to(
        device
    )
    ...
    final_model = Unbounded_Metalog_Model(init_a=torch.ones([channels, 7])).to(
        device
    )
